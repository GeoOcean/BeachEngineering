...
def theta_0(df_waves, orientation):
    ...
    if (orientation > 90) & (orientation < 270):
        df_waves.loc[(df_waves['mwd'] < min_range) | (df_waves['mwd'] > max_range), 'swh'] = 0
    else:
        df_waves.loc[(df_waves['mwd'] > max_range) & (df_waves['mwd'] < min_range), 'swh'] = 0

    df_waves['theta_0'] = [min_dist(orientation, i) for i in df_waves['mwd']]
...
def snell_mono_prop(df_waves, h0, h1, m, orientation):
    ...
    df_waves['L0'] = ((9.81)* df_waves['mwp']**2)/(2 * np.pi)  
